...
def parsing_header_data(header):
    ...
    while (txt_decode[Line_num].split(" ")[0] != "DATA"):
        # print("$", txt_decode[Line_num].split(" "), "$")
...
def parsing_PCD_data(PCD):
    ...
    while (1):
        try:
            byte_len = 4 * 4 + 1 * 5
            x, y, z, intensity, t, reflectivity, ring, ambient, range = struct.unpack("ffff BBBBB", PCD[start: start + byte_len])  # B:부호없는 정수, c:문자
            lines.append('{:.6f} {:.6f} {:.6f} {:.6f} {} {} {} {} {}'.format(x, y, z, intensity, t, reflectivity, ring, ambient, range))
...
def parsing_bin_PCD(directory_path = os.getcwd()): # args가 없으면 코드가 위치한 디렉토리에서 검색,변환
    ...
    file_list = [file for file in file_list if file.endswith("2.pcd")] ## 지정된 디렉토리 내 pcd 파일만 검색
    file_list = [file for file in file_list if file.startswith("1_")]  ## 지정된 디렉토리 내 pcd 파일만 검색
    # file_list = ["2_20210714_161648_000030_56_bin.pcd"]
    for i in file_list: #디렉토리 내 pcd 확장자 파일 대상으로 변환 시작 (ascii binary 구분없이 다 처리함)(근데 ascii 타입 pcd는 에러가 난다)
        ...
        txt = Origin_pcd_f.read(0xfc) # 헤더까지 하드코딩으로 짜르기. 샘플로 제공된 pcd 파일은 전부 0x135위치까지 header가 차지하고 있기 때문에.
        ...
        with open (file_name[0] + "_ascii.pcd", 'w+') as f:
            f.write(HEADER.format(POINTS, POINTS)+'\n'.join(lines))
    exit(1)
