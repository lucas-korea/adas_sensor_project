...
def parsing_PCD_data(PCD):
    ...
    while (1):
        try:
            # byte_len = 4 * 9 + 1 * 1
            # SF6, SF5, SF4, SF3, SF2, SF1, x, y, z, _ = struct.unpack("fff fff fff c", PCD[start : start + byte_len]) # B:부호없는 정수, c:문자
            # lines.append('{:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {}'.format(SF6, SF5, SF4, SF3, SF2, SF1, x, y, z, _))
            byte_len = 4 * 9 + 1 * 4
            SF6, SF5, SF4, SF3, SF2, SF1, x, y, z, c1, c2, c3, c4 = struct.unpack("fffffffffBBBB", PCD[start: start + byte_len])  # B:부호없는 정수, c:문자
...
def parsing_bin_PCD(directory_path = os.getcwd()): # args가 없으면 코드가 위치한 디렉토리에서 검색,변환
    ...
    for i in file_list: #디렉토리 내 pcd 확장자 파일 대상으로 변환 시작 (ascii binary 구분없이 다 처리함)(근데 ascii 타입 pcd는 에러가 난다)
        ...
        txt = Origin_pcd_f.read(0x135) # 헤더까지 하드코딩으로 짜르기. 샘플로 제공된 pcd 파일은 전부 0x135위치까지 header가 차지하고 있기 때문에.
