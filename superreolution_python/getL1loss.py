import numpy as np
print("data loading...")
origin_data = np.load("C:\\Users\\jcy37\\PycharmProjects\\superreolution_python\\20220913_429.npy")
print("load origin data done")
nomalize_parameter = 100
pred_data = np.load("C:\\Users\\jcy37\\PycharmProjects\\superreolution_python\\20220913_429-UNet-from-32-to-128_prediction.npy") * nomalize_parameter
...
LAMDA = 0.03
condition_remove = pred_data[:,:,:,1:2] > pred_data[:,:,:,0:1] * LAMDA
...
L1_loss = sum(sum(sum(abs(origin_data[:,:,:,0] - pred_data[:,:,:,0]))))
