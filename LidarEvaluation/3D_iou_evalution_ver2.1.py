            ...
            for line_num in range(file_length):
                ...
                for j in range(8):
                    try:
                        ...
                        GT_data_splited[j] = double(GT_data_splited[j].replace('<br>', ''))## 라벨링툴에서의 버그 간헐적으로 발생
...
def sort_matchin_GT_output(matching_df):
    ...
    for i in range(origin_df.shape[0]):  # 행개수(gt개수) 만큼 반복
        ...
        while (matching_df.shape[1] > Nlarge_i - 1):
            ...
            matched_col = \
            matching_df.astype(float).apply(lambda row: row.nlargest(Nlarge_i).index.values[Nlarge_i - 1], axis=1).loc[
                matching_df.astype(float).apply(lambda row: row.nlargest(Nlarge_i), axis=1).index[0]]
            # 기준 GT에 대해 Nlarge_i번째 큰 iou를 가진 output
            matched_key = matching_df[
                matching_df.astype(float).apply(lambda row: row.nlargest(Nlarge_i).index.values[Nlarge_i - 1], axis=1)
                .loc[matching_df.astype(float).apply(lambda row: row.nlargest(Nlarge_i), axis=1).index[0]]].astype(
                float).idxmax()  # 기준 GT에 대해 Nlarge_i번째 큰 iou를 가진 output 기준 가장 큰 iou를 가진 GT

            if matched_row == matched_key: ## and get_class_from_str(matched_row) == get_class_from_str(matched_col):
                break  # 가장 큰 iou와 객체 종류가 잘 매칭되면 끝
            Nlarge_i += 1
        if matched_row == matched_key: ## and get_class_from_str(matched_row) == get_class_from_str(matched_col):  # 서로 잘 매칭 됐으면 매칭된 거 삭제하기
...
def matching_GT_output(result_frame):
    ...
    for i in range(len(result_frame['output'])):
        if not chekc_isin_ROI(result_frame['output'][i]):
            cnt += 1
            continue
        matching_df.insert(i - cnt, str(result_frame['output'][i]),
                           '')  # 열을 insert할 때는 index 인자를 넣어야 하는데, for문의 iterator로는 continue 문의 상황을 반영하지 못하므로 cnt를 감하여 보정.
...
def draw_3d_Bbox(matching_df2):
    ...
    for GT in matching_df2.index:
        id, cx, cy, bz, l, w, h, yaw, GTcla = str2list(GT)
        x_corners = [l / 2 + cx, l / 2 + cx, -l / 2 + cx, -l / 2 + cx, l / 2 + cx, l / 2 + cx, -l / 2 + cx,
                     -l / 2 + cx]
        y_corners = [w / 2 + cy, w / 2 + cy, w / 2 + cy, w / 2 + cy, -w / 2 + cy, -w / 2 + cy, -w / 2 + cy,
                     -w / 2 + cy]
        z_corners = [h / 2 + bz + h / 2, -h / 2 + bz + h / 2, -h / 2 + bz + h / 2, h / 2 + bz + h / 2,
                     h / 2 + bz + h / 2, -h / 2 + bz + h / 2, -h / 2 + bz + h / 2,
                     h / 2 + bz + h / 2]
    ...
    for output in matching_df2.columns.values:
        id, cx, cy, bz, l, w, h, yaw, outputcla = str2list(output)
        x_corners = [l / 2 + cx, l / 2 + cx, -l / 2 + cx, -l / 2 + cx, l / 2 + cx, l / 2 + cx, -l / 2 + cx,
                     -l / 2 + cx]
        y_corners = [w / 2 + cy, w / 2 + cy, w / 2 + cy, w / 2 + cy, -w / 2 + cy, -w / 2 + cy, -w / 2 + cy,
                     -w / 2 + cy]
    ...
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    # ax.text(-27, 0, 23, , fontdict=font)
    ax2.set_xlim(0, 10)
    ax2.set_ylim(10, 25)
    ax2.text(0,20, "path : " + '\\'.join(output_path.split('\\')[-5:-1]), fontdict = font)
    ax2.text(0,19, '       '+'\\'.join(GT_path.split('\\')[-5:-1]), fontdict=font)
    # ax2.text(0,17, "GT : {}\nOutput : {}".format(GTcla, outputcla))
    ax2.text(0,16, "iou : {}".format(matching_df2.values[0]))
    # if matching_df2.values[0][0] > 0.5 and GTcla == outputcla : ax2.text(0,14, "pass")
    # else : ax2.text(0,15, "fail")
    ax2.text(0,13, "frame : {} / {}".format(draw_cnt, file_len))
    ax2.axis('off')
    plt.pause(0.001)
    gc.collect()
...
# object가 ROI에 해당하는지, 최소 사이즈 이상인지 True or False 판단
...
if __name__ == '__main__':
    output_path = select_folder("output_path")
    # output_path = "C:\\Users\\jcy37\\Desktop\\tta_SW평가\\tta_evaluation_modified\\output_datasets"
    GT_path = select_folder("GT_path")
    ...
    for path, dirs, files in os.walk(output_path):
        if 'label' in dirs:
            ...
            GT_path = output_path.replace('output_datasets','ground_datasets')
            print(GT_path)
            print(output_path)
            # output_path = "C:\\Users\\jcy37\\Desktop\\tta_SW평가\\tta_evaluation_modified\\ground_datasets\\daynight\\car\\day_car_1\\label"
            # GT_path = "C:\\Users\\jcy37\\Desktop\\tta_SW평가\\tta_evaluation_modified\\output_datasets\\daynight\car\\day_car_1\\label"
            output_files = os.listdir(output_path)
            file_len = len(output_files)
            result = pd.merge(make_GT_df(GT_path_=GT_path), make_output_df(output_path_=output_path), on="filename",
                              how="outer")
            result["my_IOU"] = None
            result["TP"] = None
            result["FP"] = None
            ...
            result.to_csv(output_path.split('\\')[-2] + ".csv")
