...
def cal_lidar_pos():
    ...
    y_ = y_[:, 0::4]
    z_ = z_[:, 0::4]
    reflectivity32ch = reflectivity[:, 0::4]
    print(Azimuth_sum)
    print(Azimuth.shape)
    print(angle.shape)
    # print(print(distance.shape), distance)
    # print(np.reshape([distance[:, 0::4]], (1, 1024,32, 1)) )
    exit(1)
    np.save("result", np.reshape([distance[:, 0::4]], (1, 1024,32, 1)))
    result = np.stack([x_, y_, z_, reflectivity32ch], axis=-1).reshape(-1, 4)
...
def make_bin_PCDfile(point_cloud, lidar_list_dir_path, ymd, hms, frame_num, tick_ct):
    ...
    with open(lidar_list_dir_path + "\\" + ymd + "_" + hms + "_" + '{0:06d}'.format(int(frame_num)) + "_R.pcd", 'ab') as f:
        # point_cloud = np.round(point_cloud, 4) # 정밀성을 위해 삭제
            ...
            while 1:
                try:
                    ...
                    if (frame_i % 10 == 0  or 1):
                        ...
                        make_ascii_PCDfile(point_cloud, lidar_list_dir_path, ymd, hms, frame_i, tick_ct)  # point cloud data를 pcd file로 변환
                    frame_i = frame_i + 1
                    packets = []
                except Exception as e:
                    print("error : ", e)
