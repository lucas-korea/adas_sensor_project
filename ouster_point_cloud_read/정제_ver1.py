...
#data encoder 값이 309.375도 부터 시작하는지 확인
def find_start(data):
    index = 0
...
#data encoder 값이 0도 부터 시작하는지 확인
def find_0deg(data):
    index = 0
    index = index + 8  # timestamp
    encoder = data[index + 4: index + 8]
...
def make_ascii_PCDfile(point_cloud, lidar_list_dir_path, ymd, hms, frame_num, tick_ct):
    ...
    with open(lidar_list_dir_path + "\\" + ymd + "_" + hms + "_" + '{0:06d}'.format(int(frame_num)) + "_" + '{0:06d}'.format(int(tick_ct)) + ".pcd", 'a') as f:
        for i in range(len(point_cloud)):
            for j in range(3):
                point_cloud[i][j] = round(point_cloud[i][j], 4)
                ...
                try:
                    ...
                    for i in range(64):
                        ...
                        if find_0deg(data):
                            ...
                            frame_number = round(1.3315 * int(frame_number))
                            tick_ct = header[2]
                    ...
                    if frame_i % 10 == 0 or frame_i % 10 == 3 or frame_i % 10 == 6 or 1:
                        ...
                        with open(lidar_list_dir_path + "\\" + "image_frame_number.txt", 'a') as frame_f:
                            frame_f.write(str(frame_number))
                            frame_f.write('\n')
                        make_ascii_PCDfile(point_cloud, lidar_list_dir_path, ymd, hms, frame_number, tick_ct)  # point cloud data를 pcd file로 변환
                    frame_i = frame_i + 1
                    packets = []
                except Exception as e:
                    print("error : ", e)
